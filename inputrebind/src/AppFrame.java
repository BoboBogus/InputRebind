...
public class AppFrame extends JFrame implements NativeKeyListener, NativeMouseInputListener{
    ...
    public void nativeKeyPressed(NativeKeyEvent e) {
        KeyEvent event = translate(e);
        int keyCode = event.getKeyCode();
        if(listenBind){
            Bind = keyCode;
            buttonBind.setText(KeyEvent.getKeyText(keyCode));
            listenBind = false;
        ...
        if(listenReplace){
            isKey = 1;
            Replace = keyCode;
            buttonReplace.setText(KeyEvent.getKeyText(keyCode));
            listenReplace = false;
        }
        if (enable){
            if(keyCode == Bind){
    ...
    //translation hell
    protected KeyEvent translate(NativeKeyEvent e) {
