...
public class Rebinder implements NativeKeyListener, NativeMouseInputListener{
    boolean listen;
    ...
    int current = -1;
    public List<RebindSet> sets = new ArrayList<RebindSet>();
//first is bind second is replace

    public Rebinder() throws AWTException{
        robot = new Robot();
        ...
        //need to await for change
        public void listen(RebindSet set, boolean forReplace, JButton button) throws InterruptedException{
            int past = current;
                listen = true;
            synchronized(this){
                while(past == current){
            ...
            if(isKey==1){
                button.setText(KeyEvent.getKeyText(current));
            }
            else if(isKey==2){
                button.setText("Mouse" +current);
            }
            current = -1;
        }

        public void nativeKeyPressed(NativeKeyEvent e) {
            int translatedKeyCode = translate(e);
            if(listen){
                synchronized(this){
                    current = translatedKeyCode;
                    isKey = 1;
                    listen = false;
                    this.notifyAll();
                }
            ...
            for(int x = 0; x <sets.size(); x++){
                if(sets.get(x).Bind == translatedKeyCode){
                    if(sets.get(x).isKey == 1){
                        robot.keyPress(sets.get(x).Replace);
                    }
                    else if(sets.get(x).isKey == 2){
                        robot.mousePress(sets.get(x).Replace);
                    }
                }
            }
        ...
        public void nativeMousePressed(NativeMouseEvent e) {
            ...
            for(int x = 0; x <sets.size(); x++){
                if(sets.get(x).Bind == e.getButton()){
                    if(sets.get(x).isKey == 1){
                        robot.keyPress(sets.get(x).Replace);
                    }
                    else if(sets.get(x).isKey == 2){
        ...
        public void nativeMouseReleased(NativeMouseEvent e) {
            ...
            for(int x = 0; x <sets.size(); x++){
                if(sets.get(x).Bind == e.getButton()){
                    if(sets.get(x).isKey == 1){
                        robot.keyRelease(sets.get(x).Replace);
                    }
                    else if(sets.get(x).isKey == 2){
                        robot.mouseRelease(sets.get(x).Replace);
                    }
                }
            }
        }
        ...
        public void nativeKeyRelease(NativeKeyEvent e){
            ...
            for(int x = 0; x <sets.size(); x++){
                if(sets.get(x).Bind == translate(e)){
                    if(sets.get(x).isKey == 1){
                        robot.keyRelease(sets.get(x).Replace);
                    }
                    else if(sets.get(x).isKey == 2){
        ...
        //translation hell
        protected int translate(NativeKeyEvent e) {
